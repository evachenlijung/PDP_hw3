...
# 使用 Bash 參數展開設定預設值
# 如果沒有傳入 $1，就使用預設的 old_csv；沒有 $2，就使用預設的 new_csv
OLD_CSV="${1:-~/hw3/eva/logs/q1_mpi/q1_mpi_new.csv}"
NEW_CSV="${2:-~/hw3/eva/logs/q1_mpi/q1_mpi_v3.csv}"

echo "=== MPI Performance Comparator ==="
echo "Old CSV: $OLD_CSV"
echo "New CSV: $NEW_CSV"
echo "=================================="

# 將變數傳遞給 Python
python3 - "$OLD_CSV" "$NEW_CSV" << 'EOF'
import sys
import csv
import os
import warnings
...
# 🌟 透過 os.path.expanduser 確保 Bash 傳進來的 '~' 符號能被正確展開為絕對路徑
old_csv = os.path.expanduser(sys.argv[1])
new_csv = os.path.expanduser(sys.argv[2])
...
print(f"\n{'testcase':<25} {'npernode':>8} {'nprocs':>6} "
...
fig.suptitle("Performance Comparison (Old vs New)", fontsize=14)
...
for ax, tkey, tlabel in zip(axes, time_keys, time_labels):
    ...
    for n in names:
        ...
        ax.plot(x, old_vals, marker="o", color=mcolors.to_hex(light),
                ...
                label=f"{n} (Old)" if n == names[0] else "_nolegend_", zorder=3)
        # new: solid full color
        ax.plot(x, new_vals, marker="D", color=color,
                linewidth=2, markersize=7, linestyle="-",
                label=f"{n} (New)" if n == names[0] else "_nolegend_", zorder=4)
...
for n in names:
    ...
    legend_handles.append(
        Line2D([0], [0], color=mcolors.to_hex(light), linewidth=1.5,
               linestyle="--", marker="o", markersize=5, label=f"{n} Old"))
    legend_handles.append(
        Line2D([0], [0], color=color, linewidth=2,
               linestyle="-", marker="D", markersize=7, label=f"{n} New"))
