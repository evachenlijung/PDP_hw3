...
Q1_MPI_NO_SM_CSV=~/hw3/eva/logs/q1_mpi_no_sm.csv
echo "testcase,npernode,nprocs,image_size,total_weight,target_per_proc,scatter_time,avg_render_time,composite_time,total_time,img_size_vs_base,weight_vs_base,time_vs_base" > $Q1_MPI_NO_SM_CSV
...
for bin in "${TESTCASES[@]}"; do
    ...
    for npernode in "${NPERNODE_LIST[@]}"; do
        ...
        echo "=== Running MPI: $name npernode=$npernode (total $nprocs procs) ==="
...
for bin in "${TESTCASES[@]}"; do
    ...
    for npernode in "${NPERNODE_LIST[@]}"; do
        ...
        echo "$name,$npernode,$nprocs,$image_size,$total_weight,$target,$scatter,$avg_render,$composite,$total,$img_vs_base,$wt_vs_base,$time_vs_base" >> $Q1_MPI_NO_SM_CSV
    done
done

echo "=== Done. Results written to $Q1_MPI_NO_SM_CSV ==="
...
csv_path = os.path.join(logs_dir, "q1_mpi_no_sm.csv")
...
fig.suptitle("MPI Renderer: Timing Breakdown vs npernode by Testcase (No Shared Memory)", fontsize=14)
...
for col, (ax, tkey, tlabel, marker, fmt) in enumerate(zip(axes, time_keys, time_labels, markers, time_fmt)):
    ...
    for xi, np_node in enumerate(NPERNODE_LIST):
        ...
        y_bottom = ax.get_ylim()[1] * 0.55
        step = (y_top - y_bottom) / (len(col_points) - 1) if len(col_points) > 1 else 0

        for rank_idx, (t_val, speedup, color) in enumerate(col_points):
            y_pos = y_top - step * rank_idx
            # shift npernode=2 labels lower to avoid overlap with npernode=1 and 4
            if xi == 1:
                y_pos -= (y_top - y_bottom) * 0.15
            ax.annotate(
                f"{fmt.format(t_val)}\n{speedup:.2f}x",
                xy=(x[xi], t_val),
                xytext=(x[xi], y_pos),
...
plt.tight_layout()
outpath = os.path.join(logs_dir, "q1_mpi_no_sm_plot.png")
