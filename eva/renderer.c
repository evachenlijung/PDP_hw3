...
int main(int argc, char **argv) {
    ...
    for (uint64_t i = 0; i < count; ++i) {
        ...
        double w = (double)radius * (double)radius;
        totalw += w;

        // Determine integer bounding box of pixels to test
        ...
        float Ca = 1.0f; /* fully opaque (alpha removed from format) */
        ...
        for (int y = ymin; y <= ymax; ++y) {
            for (int x = xmin; x <= xmax; ++x) {
                ...
                if ((dx*dx + dy*dy) <= r2) {
                    ...
                    float Pg = img[idx + 1];
                    float Pb = img[idx + 2];
                    img[idx + 0] = Ca * Cr + (1.0f - Ca) * Pr;
                    img[idx + 1] = Ca * Cg + (1.0f - Ca) * Pg;
                    img[idx + 2] = Ca * Cb + (1.0f - Ca) * Pb;
                }
            }
        } 
    }
