...
mkdir -p ~/hw3/eva/output/q2_new
mkdir -p ~/hw3/eva/logs/q2_new

Q2_CSV=~/hw3/eva/logs/q2_new/q2_new_strong_scaling.csv
...
for bin in "${TESTCASES[@]}"; do
    ...
    for npernode in "${NPERNODE_LIST[@]}"; do
        nprocs=$((npernode * 4))
        outpng=~/hw3/eva/output/q2_new/${name}_npernode${npernode}.png
        logfile=~/hw3/eva/logs/q2_new/${name}_npernode${npernode}.log
...
logs_dir = os.path.expanduser("~/hw3/eva/logs/q2_new")
csv_path = os.path.join(logs_dir, "q2_new_strong_scaling.csv")
...
plt.tight_layout()
outpath = os.path.join(logs_dir, "q2_new_strong_scaling.png")
