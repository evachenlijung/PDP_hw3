...
# 建立輸出與日誌目錄
mkdir -p ~/hw3/eva/output/q3_v3
mkdir -p ~/hw3/eva/logs/q3_v3

Q3_CSV=~/hw3/eva/logs/q3_v3/q3_v3_weak_scaling.csv
...
for name in "${TESTCASES[@]}"; do
    ...
    for npernode in "${NPERNODE_LIST[@]}"; do
        nprocs=$((npernode * 4))
        outpng=~/hw3/eva/output/q3_v3/${name}_npernode${npernode}.png
        logfile=~/hw3/eva/logs/q3_v3/${name}_npernode${npernode}.log
...
logs_dir = os.path.expanduser("~/hw3/eva/logs/q3_v3")
csv_path = os.path.join(logs_dir, "q3_v3_weak_scaling.csv")
...
plt.tight_layout()
outpath = os.path.join(logs_dir, "q3_v3_weak_scaling.png")
