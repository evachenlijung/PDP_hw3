...
# mkdir -p ~/hw3/eva/output/q2_v3
# mkdir -p ~/hw3/eva/logs/q2_v3

# Q2_CSV=~/hw3/eva/logs/q2_v3/q2_v3_strong_scaling.csv
...
#         nprocs=$((npernode * 4))
#         outpng=~/hw3/eva/output/q2_v3/${name}_npernode${npernode}.png
#         logfile=~/hw3/eva/logs/q2_v3/${name}_npernode${npernode}.log
...
INPUT_CSV="${1:-logs/q1_mpi/q1_mpi_v3.csv}"

if [ ! -f "$INPUT_CSV" ]; then
    echo "❌ 錯誤: 找不到資料檔案 $INPUT_CSV"
    echo "請確認 Q1 測試已經跑完，或手動指定 CSV 路徑。"
    exit 1
fi

echo "📊 開始分析數據: $INPUT_CSV"

python3 - "$INPUT_CSV" << 'EOF'
# python3 << 'EOF'
import os
import csv
import warnings
...
logs_dir = os.path.expanduser("~/hw3/eva/logs/q2_v3")
csv_path = os.path.join(logs_dir, "q2_v3_strong_scaling.csv")
...
plt.tight_layout()
outpath = os.path.join(logs_dir, "q2_v3_strong_scaling.png")
